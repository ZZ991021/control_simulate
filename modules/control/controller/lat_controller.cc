...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  */
  matrix_a_(0, 1) = 1.0;
  matrix_a_(1, 2) = (cf_ + cr_ ) / mass;
  matrix_a_(2, 3) = 1.0;
  matrix_a_(3, 2) = (cf_ * lf_ - cr_ * lr_) / iz_;

  matrix_a_coeff_ = Matrix::Zero(matrix_size, matrix_size);
  matrix_a_coeff_(1, 1) = - (cf_ + cr_ ) / mass;
  matrix_a_coeff_(1, 3) = (cr_ * lr_ - cf_ * lf_ ) / mass;
  matrix_a_coeff_(3, 1) = (cr_ * lr_ - cf_ * lf_ ) / iz_;
  matrix_a_coeff_(3, 3) = - (lf^2 * cf_ + lr^2 * cr_) / iz_;
  ...
  if (FLAGS_enable_gain_scheduler) {
    ...
    matrix_q_updated_(2, 2) =
        matrix_q_(2, 2) * heading_err_interpolation_->Interpolate(
                              std::fabs(vehicle_state->linear_velocity()));
    common::math::SolveLQRProblem(matrix_adc_, matrix_bdc_, matric_q_,
  ...
  if (injector_->vehicle_state()->gear() == canbus::Chassis::GEAR_REVERSE) {
    ...
    steer_angle_feedforwardterm =
        (wheelbase_ / ref_curvature + kv * v * v / ref_curvature -
         matrix_k(0,2) *
             (lf_ / 2.0 / cf_ -
              lf_ * mass_ * v * v / 2 / cr_ / ref_curvature / wheelbase)) *
